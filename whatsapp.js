...
const excelFilePath = 'messages.xlsx';
...
if (fs.existsSync(excelFilePath)) {
    ...
    worksheet = XLSX.utils.aoa_to_sheet([["Number", "Message", "Timestamp"]]);
...
client.on('message', async msg => {
    ...
    // Convert the sheet to JSON for easy manipulation
    const sheetData = XLSX.utils.sheet_to_json(worksheet, { header: 1 });

    // Check if the sender's number already exists
    const existingRowIndex = sheetData.findIndex(row => row[0] === senderNumber);

    if (existingRowIndex === -1) {
        // Number not found, add a new row
        const newRow = [senderNumber, messageContent, timestamp];
        sheetData.push(newRow);
    } else {
        // Number found, update the message and timestamp (optional)
        sheetData[existingRowIndex][1] = messageContent;
        sheetData[existingRowIndex][2] = timestamp;
    }

    // Update the worksheet and save the file
